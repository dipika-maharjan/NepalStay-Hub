...
export default function AdminBookingsPage() {
  ...
  const getAccommodationName = (booking: Booking) => {
    if (typeof booking.accommodationId === "object") {
      return (
        booking.accommodationId.name || booking.accommodationId.title || "N/A"
      );
    }
    return "N/A";
  };

  const getRoomTypeName = (booking: Booking) => {
    if (typeof booking.roomTypeId === "object") {
      return booking.roomTypeId.name || "N/A";
    }
    return "N/A";
  };
  ...
  const handleCancel = async (bookingId: string, accommodationName: string) => {
    if (
      !confirm(
        `Are you sure you want to cancel booking for ${accommodationName}?`,
      )
    ) {
      return;
    }
                      ...
                      {booking.bookingStatus !== "cancelled" && (
                        <button
                          onClick={() =>
                            handleCancel(
                              booking._id,
                              getAccommodationName(booking) === "N/A"
                                ? "this accommodation"
                                : getAccommodationName(booking),
                            )
                          }
