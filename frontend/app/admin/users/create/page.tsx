    ...
    try {
      ...
      if (response.success) {
        ...
        if (response.data.imageUrl) {
          setImagePreview(normalizeImageUrl(response.data.imageUrl));
        }
      }
    } catch (err: any) {
      ...
      <div className="bg-white rounded-lg shadow p-4 sm:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Link
              href={`/admin/users/${id}`}
              className="w-full sm:flex-1 px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition text-center"
            >
              Cancel
            </Link>
          </div>
