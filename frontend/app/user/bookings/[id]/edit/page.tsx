...
export default function EditBookingPage({ params }: PageProps) {
  ...
  const normalizeBookingDetails = (payload: unknown): BookingDetails => {
    if (!payload || typeof payload !== "object") {
      return { booking: undefined, extras: [] };
    }
    ...
    if (isBookingLike) {
      return { booking: payload as any, extras: [] };
    }

    if (candidate.booking && typeof candidate.booking === "object") {
      return {
        booking: candidate.booking as any,
  ...
  const fetchBooking = async () => {
    try {
      ...
      if (response.success) {
        const data = normalizeBookingDetails(response.data ?? response);
        setBookingData(data);

        const booking = data.booking;
        if (!booking) {
          throw new Error("Booking data is unavailable");
        }

        setSelectedRoomType(
          String(booking.roomTypeId?._id || booking.roomTypeId || ""),
        );
        setCheckIn(
          booking.checkIn
        ...
        setRoomsBooked(booking.roomsBooked || 1);
        setSpecialRequest(booking.specialRequest || "");

        const accommodationId = String(
          booking.accommodationId?._id || booking.accommodationId || "",
        );
        ...
        if (roomTypesRes.success) {
          const roomTypesList = Array.isArray(roomTypesRes.data)
            ...
            : [roomTypesRes.data];
          setRoomTypes(
            roomTypesList.filter((rt: RoomType) => rt.totalRooms > 0),
          );
        }
                ...
                >
                  ...
                  {roomTypes.map((roomType) => (
                    <option key={roomType._id} value={roomType._id}>
                      {roomType.name} | Rs.{" "}
                      {roomType.pricePerNight.toLocaleString()}/night | Max{" "}
                      {roomType.maxGuests} guests | {roomType.totalRooms} rooms
                      available
                    </option>
                  ))}
                </select>
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between text-xs mt-2">
                ...
                <span>
                  Rs.{" "}
                  {calculateTax(
                    calculateRoomTotal() + calculateExtrasTotal(),
                  ).toLocaleString()}
                </span>
              </div>
