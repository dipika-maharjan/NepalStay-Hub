...
export const getAllAccommodations =
  async (): Promise<AccommodationResponse> => {
    try {
  ...
  async (): Promise<AccommodationResponse> => {
    try {
      ...
      throw new Error(
        err.response?.data?.message ||
          err.message ||
          "Failed to fetch active accommodations",
      );
    }
  };
...
export const getAccommodationsByPriceRange = async (
  minPrice: number,
  maxPrice: number,
): Promise<AccommodationResponse> => {
  try {
    const response = await axios.get(
      `${API.ACCOMMODATION.PRICE_RANGE}?minPrice=${minPrice}&maxPrice=${maxPrice}`,
    );
    return response.data;
  } catch (err: any) {
    throw new Error(
      err.response?.data?.message ||
        err.message ||
        "Failed to fetch accommodations by price range",
    );
  }
};
