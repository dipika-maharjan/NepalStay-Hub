...
const normalizeBookingResponse = (payload: any) => ({
  success: true,
  data: payload?.bookings ?? payload?.booking ?? payload?.data ?? payload,
...
export const getMyBookings = async () => {
  try {
    ...
    throw new Error(
      error.response?.data?.message || error.message || "Get bookings failed",
    );
  }
};
...
export const cancelBooking = async (bookingId: string) => {
  try {
    ...
    throw new Error(
      error.response?.data?.message || error.message || "Cancel booking failed",
    );
  }
};
